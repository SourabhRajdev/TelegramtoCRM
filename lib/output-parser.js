...
// v7 schema — matches ARIA_v7_System_Prompt.md Output Contract exactly
const ariaResponseSchema = z.object({
  ...
  message: z.string().describe(
    'Read: empty string. Write confirmation pending: short "X to Y. Proceed?". Greeting: "Hey! What do you need?". Clarify: one specific question with two named options.'
  ),

  // SAFETY: Write operations require explicit user confirmation before executing
  awaiting_confirmation: z.boolean().describe(
    'true on first write intent — queries must be empty, message must ask "...Proceed?". false after user confirms or for all reads/greetings.'
  ),
});
...
function normalizeManualParse(parsed) {
  ...
  return {
    ...
    awaiting_confirmation: parsed.awaiting_confirmation || false,
  };
}
...
function validateAgentOutput(output) {
  ...
  if (!output.reasoning) {
    ...
    if (wordCount < 20) {
      reasons.push(`Reasoning is too short (${wordCount} words — minimum 20 words required).`);
    }
  }
