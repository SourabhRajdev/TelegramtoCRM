...
function validateAgentOutput(output) {
  ...
  // Filtered intents must have filters (unless person_name is the only filter)
  if (output.intent === 'list_filtered') {
    ...
    if (!hasPersonName && !hasFilters) {
      reasons.push('Intent is "list_filtered" but no filters or person_name extracted.');
    }
  }
