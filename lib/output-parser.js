...
let _parser = null;
let _formatInstructions = null;

async function getParser() {
  if (_parser) return _parser;
  const { StructuredOutputParser } = await import('langchain/output_parsers');
  _parser = StructuredOutputParser.fromZodSchema(ariaResponseSchema);
  return _parser;
}

async function getFormatInstructions() {
  if (_formatInstructions) return _formatInstructions;
  const parser = await getParser();
  _formatInstructions = parser.getFormatInstructions();
