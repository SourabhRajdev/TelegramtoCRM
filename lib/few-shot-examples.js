...
const EXAMPLES = [
  ...
  // ── CREATE ITEM ────────────────────────────────────────────
  {
    ...
    input: 'add lead Omar Saeed +971509876543 from referral assigned to Sourabh',
    output: JSON.stringify({
      reasoning: 'User wants to create a new lead on the Sales board. Person: Omar Saeed. Phone: +971509876543. Source channel: Referral. Assigned AE: Sourabh. Generating create_item mutation with semantic column values (phone, source, assigned_ae) which will be translated to real Monday.com column IDs.',
      intent: 'create_item',
      entities: {
        person_name: 'Omar Saeed',
        board: 'sales',
        filters: [],
        values_to_set: { phone: '+971509876543', source: 'Referral', assigned_ae: 'Sourabh' },
      },
      action_type: 'write',
      needs_data: true,
      queries: ['mutation { create_item(board_id: {{SALES_BOARD_ID}}, group_id: \\"{{SALES_GROUP_ID}}\\", item_name: \\"Omar Saeed\\", column_values: \\"{\\\\\\"phone\\\\\\":{\\\\\\"phone\\\\\\":\\\\\\"+971509876543\\\\\\",\\\\\\"countryShortName\\\\\\":\\\\\\"AE\\\\\\"},\\\\\\"source\\\\\\":{\\\\\\"label\\\\\\":\\\\\\"Referral\\\\\\"},\\\\\\"assigned_ae\\\\\\":\\\\\\"Sourabh\\\\\\"}\\") { id name } }'],
      message: 'Creating lead Omar Saeed — phone +971509876543, source Referral, AE Sourabh',
      follow_up: '',
    }),
  },
  ...
  // ── STAFF QUERY (tasks) ────────────────────────────────────
  {
    ...
    output: JSON.stringify({
      reasoning: 'User wants to see tasks assigned to Yash. Board is Staff (keyword: "working on" implies staff tasks). Person name: "Yash" extracted as ENTITY (not filter). Staff board uses item codes (STF-XXX), so I must fetch all items and the system filters by person name in column values. Filters array is EMPTY because person_name is an identity anchor, not an attribute filter.',
      intent: 'list_filtered',
      entities: {
        person_name: 'Yash',
        board: 'staff',
  ...
  // ── GREETING ───────────────────────────────────────────────
  {
    ...
    input: 'hey',
    output: JSON.stringify({
      reasoning: 'Simple greeting with no operational intent, no board keywords, no data request. Just a greeting. Responding as ARIA — brief, ready to work. No filler, no "How can I help you today?" nonsense.',
      intent: 'greeting',
  ...
  // ── FOLLOW-UP (context-aware) ──────────────────────────────
  {
    ...
    output: JSON.stringify({
      reasoning: 'Follow-up to a previous query. User said "the ones" — referring to the last result set. Adding a pricing filter: under 3000 AED. The previous query was about artists (from conversation context). MERGING previous filters with new constraint. Re-fetching artists board with the pricing filter ADDED to the existing filter set. This is context locking — I must preserve previous intent and add new constraints, not reset the query.',
      intent: 'follow_up',
      entities: {
        person_name: '',
        board: 'artists',
  ...
  // ── CLARIFY (ambiguous) ────────────────────────────────────
  {
    ...
    input: 'update Ravi',
    output: JSON.stringify({
      reasoning: 'User wants to update "Ravi" but did not specify which field to change or which Ravi (could be Ravi Khanna on Sales or Ravi Sharma on Artists). Need to ask SPECIFIC clarification with context. NOT generic "Can you clarify?" but targeted question with options. This is intelligent resolution — I attempted to interpret (found multiple Ravis), now offering specific choices.',
      intent: 'clarify',
  ...
  {
    intent_category: 'list_all',
    keywords: ['tell', 'about', 'leads', 'clients', 'our', 'top', 'give', 'show', 'me'],
    input: 'tell me about our leads',
    output: JSON.stringify({
      reasoning: 'User wants to see leads from the Sales board. Natural language request — "tell me about" means show/list. "leads" maps to the Sales board. No specific filters mentioned. Fetching all items from sales board, system displays them.',
      intent: 'list_all',
      entities: { person_name: '', board: 'sales', filters: [] },
      action_type: 'read',
      needs_data: true,
  ...
  {
    intent_category: 'list_filtered',
    keywords: ['top', 'best', 'leads', 'clients', '5', 'recent', 'latest', 'newest'],
    input: 'tell me 5 top leads of ours',
    output: JSON.stringify({
      reasoning: 'User wants the top 5 leads from the Sales board. "leads" maps to Sales. "top" suggests they want leads sorted by priority or status. No explicit status filter — fetching all sales items, system shows first results. The limit of 5 is a display preference, not a data filter.',
      intent: 'list_all',
      entities: { person_name: '', board: 'sales', filters: [] },
      action_type: 'read',
      needs_data: true,
  ...
  // ── TOPIC CHANGE / CONVERSATION RESET ──────────────────────
  {
    intent_category: 'greeting',
    keywords: ['no', 'nope', 'nevermind', 'forget', 'cancel', 'stop', 'not', 'that'],
    input: 'no',
    output: JSON.stringify({
      reasoning: 'User rejected the previous clarification or changed topic. This is a conversational reset — not a data query, not a follow-up. Acknowledging and offering to help with something else. No need to continue the previous thread.',
      intent: 'chitchat',
      entities: { person_name: '', board: 'unknown', filters: [] },
      action_type: 'chat',
      needs_data: false,
      queries: [],
      message: "OK. What do you need?",
      follow_up: '',
    }),
  },
];
...
function detectIntentSignals(input) {
  ...
  // Filtered list signals — expanded to catch natural language
  ...
  // "top N leads", "best artists", "5 top clients" — ranking/limit queries → treated as filtered
  if (/\b(top \d+|\d+ top|best|highest|lowest|newest|oldest|recent|latest)\b/i.test(input)) {
    signals.push('list_filtered');
  }
  ...
  // List all signals — expanded with natural phrasing
  if (/\b(show all|list all|everyone|full report|overview|roster|everything)\b/i.test(input)) {
    signals.push('list_all');
  }
  // "tell me about", "show me", "give me", "what are", "get me" — general data retrieval
  if (/\b(tell me|show me|give me|get me|what are|who are|pull up|bring up|let me see|display)\b/i.test(input)) {
    // If it also has filter signals, don't double-add list_all
    if (!signals.includes('list_filtered') && !signals.includes('count')) {
      signals.push('list_all');
    }
    // Always add a general "data retrieval" signal for example scoring
    signals.push('list_filtered');
  }

  // Board keyword signals — detect when user mentions a board entity
  // This doesn't classify intent, but boosts examples from the right board
  if (/\b(leads?|clients?|deals?|prospects?|pipeline|sales|inquir)/i.test(input)) {
    if (!signals.includes('list_all') && !signals.includes('list_filtered') && !signals.includes('count')) {
      signals.push('list_all');
    }
  }
  if (/\b(artists?|talent|performers?|DJs?|musicians?|dancers?|bands?|bookings?)/i.test(input)) {
    if (!signals.includes('list_all') && !signals.includes('list_filtered') && !signals.includes('count')) {
      signals.push('list_all');
    }
  }
  if (/\b(staff|team|employees?|department)/i.test(input)) {
    if (!signals.includes('list_all') && !signals.includes('list_filtered') && !signals.includes('count')) {
      signals.push('list_all');
    }
  }

  // Greeting — ONLY pure greetings (no additional entity words)
  if (/^(hey|hi|hello|sup|good morning|good evening|yo)[\s!.?]*$/i.test(input)) {
    signals.push('greeting');
  }

  // Conversation reset signals
  if (/^(no|nope|nevermind|never mind|forget it|cancel|stop|not that|start over)[\s!.?]*$/i.test(input)) {
    signals.push('greeting'); // Reset to neutral state
  }
...
function substituteBoardIds(example, boardIds) {
  ...
  if (boardIds) {
    ...
    // P0-3 FIX: Substitute real group IDs
    if (boardIds.groups) {
      output = output.replace(/\{\{SALES_GROUP_ID\}\}/g, boardIds.groups.sales || 'topics');
      output = output.replace(/\{\{ARTISTS_GROUP_ID\}\}/g, boardIds.groups.artists || 'topics');
      output = output.replace(/\{\{STAFF_GROUP_ID\}\}/g, boardIds.groups.staff || 'topics');
    }
  }
