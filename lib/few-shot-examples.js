...
const EXAMPLES = [
  ...
  // ── UPDATE ITEM ────────────────────────────────────────────
  {
    ...
    output: JSON.stringify({
      reasoning: 'User wants to update Ravi Khanna\'s pipeline status to "Contracted" on the Sales board. This is a write operation: first search by name to get item ID, then mutate the status column. Using ITEM_ID_PLACEHOLDER pattern for two-step execution.',
      intent: 'update_item',
      entities: {
  ...
  // ── CREATE ITEM ────────────────────────────────────────────
  {
    ...
    output: JSON.stringify({
      reasoning: 'User wants to create a new lead on the Sales board. Person: Omar Saeed. Phone: +971509876543. Source channel: WhatsApp. Assigned AE: Sourabh. Generating create_item mutation with column values for phone and source.',
      intent: 'create_item',
      entities: {
  ...
  // ── STAFF QUERY (tasks) ────────────────────────────────────
  {
    ...
    output: JSON.stringify({
      reasoning: 'User wants to see tasks assigned to Yash. Board is Staff (keyword: "working on" implies staff tasks). Person name: "Yash". Staff board uses item codes (STF-XXX), so I must fetch all items and the system filters by person name in column values.',
      intent: 'list_filtered',
      entities: {
        person_name: 'Yash',
        board: 'staff',
        filters: [{ field: 'person_name', operator: 'contains', value: 'Yash' }],
      },
      action_type: 'read',
      needs_data: true,
